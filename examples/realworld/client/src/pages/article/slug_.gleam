...
pub fn update(
  ...
  case msg {
    ClickedFavorite -> #(model, send_to_server(ToggleFavorite))
    ClickedFollow(username) -> #(model, send_to_server(ToggleFollow(username:)))
    UpdatedComment(val) -> #(Model(..model, comment_body: val), effect.none())
    ClickedSubmitComment -> #(
      Model(..model, comment_body: ""),
      send_to_server(SubmitComment(body: model.comment_body)),
    )
    ClickedDeleteComment(id) -> #(model, send_to_server(DeleteComment(id:)))
    ClickedDeleteArticle -> #(model, send_to_server(DeleteArticle))
