...
pub fn generate(
  ...
  [
    ...
    CodecFile("src/generated/views.gleam", emit_views_gleam(contracts)),
  ]
}
  ...
  let page_blocks =
    list.filter_map(contracts, fn(pair) {
      ...
      case contract.has_model {
        False -> Error(Nil)
        True -> Ok(emit_page_view_block(route, contract))
      }
    })
...
fn emit_page_view_block(
  ...
  let init_fn = emit_client_init(fn_suffix, prefix, contract)
