...
/// Format a structured error as the boxed message rally prints to the
  ...
  case err {
    ...
    NoEndpointsFound(server_src) ->
      error_box(
        ...
        hint: Some(
          "Add at least one handler function before running `gleam run -m rally`.",
        ),
      )
