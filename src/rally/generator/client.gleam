...
import rally/types.{type ScannedRoute, type PageContract, type ScanConfig}
...
fn app_gleam(
  ...
  let ctx_push_registration = case has_client_context {
    True ->
      "\n    let _ = transport.register_push_handler(\"__ClientContext__\", fn(raw) {\n      dispatch(ClientContextUpdate(transport.decode(dynamic.unsafe_coerce(raw))))\n    })"
    False -> ""
  }
    ...
    |> list.filter_map(fn(route) {
      case dict.get(contract_map, route.variant_name) {
        Ok(contract) if contract.has_model -> {
          ...
          Ok(
            "  " <> route.variant_name <> "PageModel(" <> alias <> ".Model)",
          )
        }
        _ -> Error(Nil)
      }
    ...
    |> list.filter_map(fn(route) {
      case dict.get(contract_map, route.variant_name) {
        Ok(contract) if contract.has_model -> {
          let alias = page_module_alias(route)
          Ok(
            "  " <> route.variant_name <> "PageMsg(" <> alias <> ".Msg)",
          )
        }
        _ -> Error(Nil)
      }
    ...
    |> list.filter_map(fn(route) {
      case dict.get(contract_map, route.variant_name) {
        Ok(contract) if contract.has_model -> {
          let pattern = route_pattern(route)
          Ok(
            "    " <> pattern <> " -> " <> route.variant_name <> "PageModel(dynamic.unsafe_coerce(dynamic.from(model)))",
          )
        }
        _ -> Error(Nil)
      }
      ...
      case dict.get(contract_map, route.variant_name) {
        Ok(contract) if contract.has_model -> {
          ...
          Ok(
            ...
            <> "      let #(new_m, e) = " <> alias <> ".update(" <> ctx_arg <> "m, msg)\n"
    ...
    |> list.filter_map(fn(route) {
      case dict.get(contract_map, route.variant_name) {
        Ok(contract) if contract.has_model -> {
          ...
          Ok(
            "    " <> vn <> "PageModel(m) ->\n"
            <> "      element.map(" <> alias <> ".view" <> view_args <> ", fn(msg) { PageMsg(" <> vn <> "PageMsg(msg)) })",
          )
        }
        _ -> Error(Nil)
      }
