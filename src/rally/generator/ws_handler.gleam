...
pub fn generate(
  ...
  let rpc_imports = case protocol {
    "json" -> ""
    _ ->
      case has_auth && !has_endpoints {
        True -> ""
        False ->
  ...
  let json_imports = case protocol {
    "json" ->
      ...
      <> json_codec_module
      <> " as json_codecs\n"
      <> string.join(json_rpc_dispatch.handler_imports(endpoints), "\n")
      <> "\n"
    _ -> ""
  }
  ...
  let time_imports = case protocol {
    "json" ->
      case has_auth {
        True ->
