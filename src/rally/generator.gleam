...
" <> option_import <> "import lustre/effect.{type Effect}
import lustre/element.{type Element}
import lustre/element/html
" <> page_imports <> ctx_import <> "\n\n" <> dispatch_page_model_type(
    routes,
    contract_map,
...
fn dispatch_init_page(
  ...
  let signature = case has_client_context {
    True ->
      "pub fn init_page(route: router.Route, client_context: client_context.ClientContext) -> #(PageModel, Effect(PageMsg)) {"
    False ->
...
fn dispatch_update_page(
  ...
  case has_client_context {
    True -> {
      "pub fn update_page(page_model: PageModel, page_msg: PageMsg, client_context: client_context.ClientContext) -> #(PageModel, Effect(PageMsg), Option(client_context.ClientContextMsg)) {\n"
      <> "  case page_model, page_msg {\n"
      <> arms
      <> "\n    _, _ -> #(page_model, effect.none(), None)\n  }\n}"
    }
    False -> {
      "pub fn update_page(page_model: PageModel, page_msg: PageMsg) -> #(PageModel, Effect(PageMsg)) {\n"
      <> "  case page_model, page_msg {\n"
      <> arms
...
fn dispatch_view_page(
  ...
  let signature = case has_client_context {
    True ->
      "pub fn view_page(page_model: PageModel, client_context: client_context.ClientContext) -> Element(PageMsg) {"
