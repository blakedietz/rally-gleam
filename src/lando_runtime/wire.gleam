...
@external(erlang, "lando_runtime_ffi", "decode")
...
@external(erlang, "lando_runtime_ffi", "decode_safe")
@external(javascript, "./rpc_ffi.mjs", "decode_safe")
fn ffi_decode_safe(data: BitArray) -> Result(a, DecodeError) {
  let _ = data
  panic as "lando_runtime/wire.ffi_decode_safe: external is missing for this target. This indicates a libero packaging bug; the function should be resolved by the @external attributes."
}
...
@external(erlang, "lando_runtime_wire_ffi", "decode_call")
fn ffi_decode_call(
  data: BitArray,
) -> Result(#(String, Int, Dynamic), DecodeError) {
  let _ = data
  panic as "lando_runtime/wire.decode_call is a server-side function, unreachable on JavaScript target"
}
...
// nolint: avoid_panic, discarded_result -- Erlang-only @external; JS fallback is unreachable
@external(erlang, "lando_runtime_wire_ffi", "variant_tag")
fn ffi_variant_tag(value: dynamic.Dynamic) -> Result(String, Nil) {
  let _ = value
  panic as "lando_runtime/wire.variant_tag is a server-side function, unreachable on JavaScript target"
}
